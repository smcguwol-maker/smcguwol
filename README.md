...
고객 소유 GitHub 저장소와 Cloudflare Pages를 위한 정적 HTML/CSS/JavaScript 홈페이지입니다. 현재는 **디자인 확인용 시안**이며 실제 서비스에 아직 배포하지 않았습니다.
...
GitHub 연결과 실제 파일 데이터 쓰기는 2026-09-15 확인했습니다. 이번 제작본은 `codex/smc-design-review` 검토 브랜치용입니다. GitHub에서 해당 브랜치를 선택하고 Code → Download ZIP으로 전체 파일을 보관할 수 있습니다. 온라인 홈페이지 주소는 고객 Cloudflare Pages 연결 후 생성합니다.

### A/B 디자인 비교

`public/design/index.html`을 열면 A안(스튜디오 소개집)과 B안(연습실 안내판)을 비교할 수 있습니다. 두 안은 실제 사진·확정 업체 정보를 사용하는 첫 화면과 공간 소개 중심의 부분 시안입니다. 기존 전체 원페이지는 `public/index.html`에 보존했습니다.

비교 페이지는 `publish: false`일 때만 생성하며, 정식 공개 빌드에서는 `public/design/` 전체가 제거됩니다. 디자인 선택과 실제 화면 검수는 아직 완료하지 않았습니다.
...
고객이 도메인 `근처연습실co.kr`의 구매 완료와 사진·방 번호, 30분당 요금·최소 1시간 예약을 확인했습니다. 전달된 네이버 플레이스 요금표를 반영했습니다. 화면 검수·Cloudflare 배포·도메인 연결은 아직 완료되지 않았습니다. 카카오톡 예약 알림 자동화는 포함하지 않았습니다. 고객 계정의 Cloudflare/DNS/네이버 검색 등록 설정은 아직 변경하지 않았습니다.
