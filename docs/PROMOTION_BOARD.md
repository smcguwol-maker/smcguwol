...
현재 상태 (2026-09-23 22:55): 화면·저장 기능 구현 및 로컬 검수 완료. 고객 이메일의 Access 로그인 허용 기록도 실제 확인했습니다. 고객이 보고한 A04는 Workers에서 지원하지 않는 공개키 요청 옵션이 원인이었으며 같은 프로젝트의 실제 런타임에서 재현·수정 후 배포했습니다. 고객 오류 화면 새로고침 후 실서비스 관리자 진입 및 저장 확인만 남았습니다. 인증번호를 제작자에게 보내지 않습니다.
...
- 현재 Production은 **7a48789f-5816-422f-9e26-f37b39c6cb34** (2026-09-23 22:55:27 KST)입니다. 홈페이지 하단의 이용 도우미는 소식 관리 링크를 가리지 않도록 링크 아래 별도 줄에 표시됩니다. Cloudflare MCP Access 목록은 실제 UI와 다른 빈목록을 반환하기도 하므로 이 응답만으로 앱을 중복 생성하지 않습니다.
- 인증 오류가 계속되면 페이지의 A01~A06 코드만 확인합니다. 토큰/쿠키/인증번호를 고객에게 요구하지 않습니다. A01 없음/크기, A02 형식, A03 claims, A04 검증키 조회, A05 키 import, A06 서명입니다. 구체적인 조건을 확인하기 전에 보안 검증을 제거하거나 고객 권한을 확대하지 않습니다.
