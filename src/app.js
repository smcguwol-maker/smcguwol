...
document.querySelectorAll('[data-video]').forEach(shell=>{shell.querySelector('button')?.addEventListener('click',()=>{const id=shell.dataset.video;if(!/^[a-zA-Z0-9_-]{11}$/.test(id))return;const frame=document.createElement('iframe');frame.title='SMC 소개 유튜브 영상';frame.src='https://www.youtube-nocookie.com/embed/'+id+'?autoplay=1';frame.allow='accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share';frame.allowFullscreen=true;frame.referrerPolicy='strict-origin-when-cross-origin';shell.replaceChildren(frame);});});
